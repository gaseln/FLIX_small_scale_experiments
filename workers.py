...
class MasterNode:
    def __init__(self, n_workers: int, alpha: [float, int, np.ndarray, list], worker: Node, dataset_name: str,
                 logreg: bool=True, ordered: bool=True, max_it: int=100, compute_smoothness_min: bool=True, **kwargs):
        ...
        if type(alpha) == float or type(alpha) == int:
            ...
            raise TypeError('Unknown type ({}) for alpha. float, int, np.ndarray or list are allowed'.format(type(alpha)))
        ...
        if self.validation is True and self.validation_ratio is None:
            val_data = load_data(kwargs.get('validation_dataset_name'), n_workers, logreg, ordered, multilabel=multilabel, d_y=d_y)
    ...
    def model_grad(self, model: str, w: np.ndarray, task_batch: [int, 'full']='full', data_batch: [int, 'full']='full', chosen_workers: [np.array, list]=None, **kwargs) -> np.ndarray:
        ...
        if model == 'expmix':
            ...
            f_kwargs = {'w': w, 'k': self.modexpmix_number_of_inner_steps, 'inner_loop_lr': self.modexpmix_inner_loop_lr, 'batch': data_batch, **kwargs}
        elif model == 'fomaml':
            f_kwargs = {'w': w, 'k': self.fomaml_number_of_inner_steps, 'inner_loop_lr': self.fomaml_inner_loop_lr, 'batch': data_batch, **kwargs}
        elif model =='reptile':
            f_kwargs = {'w': w, 'k': self.reptile_number_of_inner_steps, 'inner_loop_lr': self.reptile_inner_loop_lr, 'batch': data_batch, **kwargs}  
        else:
            raise NameError('Unknown model {}. Available models are expmix, modexpmix, fomaml and reptile.'.format(model))        
        return np.array([self.workers[i].model_stochastic_update(model=model, **f_kwargs) for i in chosen_workers]).mean(axis=0)
    
    def learning(self, model: str='expmix', w_0: np.ndarray=None, epochs: int=None, task_batch: [int, 'full']=5, data_batch: [int, 'full']='full', n_iter: int=100, save_history: bool=False, **kwargs) -> np.ndarray:
        ...
        if model == 'expmix':
            ...
            params = {'full name': 'Modified Explicit Mixture', 'outer loop': self.modexpmix_outer_loop_lr, 'update sign': -1}
        elif model == 'fomaml':
    ...
    def compute_recall_at_k(self, model: str, w: np.ndarray, counter_to_id: list, prefix: str, postfix_X: str='_X', postfix_y: str='_y', top_k=5, **kwargs) -> float:
        ...
        if not (type(self.workers[0]) == models.LogReg or type(self.workers[0] == models.MulticlassLogReg) or type(self.workers[0]) == models.LogRegNoncvx or type(self.workers[0]) == models.LinReg):
            raise TypeError('Function \'compute_recall_at_k\' can be applied only on classification task problems. Task of type {} is encountered.'.format(type(self.workers[0])))
    ...
    def compute_recall_at_k_mixture(self, w: np.ndarray, counter_to_id: list, prefix: str, postfix_X: str='_X', postfix_y: str='_y') -> float:
        ...
        postfix_y -- postfix of external labels dataset for testing (default '_y')
        """
        return self.compute_recall_at_k(model='expmix', w=w, counter_to_id=counter_to_id, prefix=prefix, postfix_X=postfix_X, postfix_y=postfix_y)

    def compute_recall_at_k_local(self, w: np.ndarray, counter_to_id: list, stepsize: float, k: int, prefix: str,  postfix_X: str='_X', postfix_y: str='_y') -> float:
        ...
        k -- number of inner steps of local model
        """    
        return self.compute_recall_at_k(model='local', w=w, counter_to_id=counter_to_id, prefix=prefix, postfix_X=postfix_X, postfix_y=postfix_y, stepsize=stepsize, k=k)
    
    def compute_loss_on_external_dataset(self, model: str, w: np.ndarray, counter_to_id: list, prefix: str, postfix_X: str='_X', postfix_y: str='_y', **kwargs) -> np.float32:
...
#         w = np.ones_like(self.workers[0].w_opt) * 0.1
    ...
    def diana_update_sparsification(self, w, h, ks, betas):
        """Return the unbiased estimate of the gradient and the table of updated memory [h_1, ..., h_n] for DIANA algorithm."""
        grad_vec = np.array([self.sparsification(self.workers[i].grad_shift(self.workers[i].x_train, self.workers[i].y_train, w) - h[i], ks[i])
                             for i in self.workers])        
    ...
    def run_diana_sparsification(self, ks, n_iter=100, model='str_cvx', exp_name='diana'):
        ...
        min_beta = min(betas)
        exp_name = 'diana' + exp_name

        
        if model == 'str_cvx':
            lr = 1.0 / (self.smoothness + 2 * max_L_omega_alpha_sq / self.n_workers + 4 * max_beta_L_omega_alpha_sq / (self.n_workers * min_beta))
        elif model == 'cvx':
            lr = 0.25 / (self.smoothness + 2 * max_L_omega_alpha_sq / self.n_workers + 4 * max_beta_L_omega_alpha_sq / (self.n_workers * min_beta))
        else:
            raise ValueError('Parameter <model> unrecognized')
        w = np.ones_like(self.workers[0].w_opt)
